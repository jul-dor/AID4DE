...
st.set_page_config(page_title="Interactive Plot Generation", layout = "wide")
st.title("Interactive Plot Generation")
st.markdown("""The Interactive Plot Generation enables you to generate new plots based on LLM suggestions""")
...
def get_visualization_suggestions(question: str, excluded_charts: list) -> list:
    ...
    system_prompt = f"""
    You are a data assistant specialized in event log analysis. Based on the user's analysis question, suggest between 3 and 10 insightful visualization ideas. Avoid suggesting the following chart types: {excluded_str}.
    Return only a clean bullet list with short explanations. Each suggestion should be unique and data-insightful.
    """
    messages = [
